...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Settings (categories, units, zones, pin)
  ...
  const sortedItems  = useMemo(()=>[...items].sort((a,b)=>{ const so={low:0,normal:1,high:2}; return (so[a.status]??1)-(so[b.status]??1)||a.name.localeCompare(b.name,"th"); }),[items]);
  ...
  const setItemStatus = (id,status) => sync(async () => {
    await update(ref(db, `items/${id}`), { status });
  });
  ...
  const confirmLog = () => {
    ...
    sync(async () => {
      ...
      items.forEach(it => {
        const d = pendingChanges[it.id]??0;
        if(d !== 0) updates[`items/${it.id}/qty`] = it.qty + d;
      });
  ...
  const delItem = id => sync(async () => {
    await remove(ref(db, `items/${id}`));
    toast$("ลบสินค้าสำเร็จ");
  });

  // ─── Settings CRUD ─────────────────────────────────────────────
  const saveSettings = (patch) => sync(async () => {
    await update(ref(db, "settings"), patch);
  });

  const addCat  = ()=>{ const v=newCatInput.trim(); if(!v)return; if(categories.includes(v)){toast$("มีอยู่แล้ว","error");return;} const n=[...categories,v]; saveSettings({categories:n}); setNewCatInput(""); toast$(`เพิ่ม "${v}" แล้ว`); };
  ...
  if (!dbReady) return (
    <div style={{ fontFamily:"'Sarabun','Noto Sans Thai',sans-serif", minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"#f5f0eb", gap:16 }}>
      ...
      <div style={{ display:"flex", gap:6 }}>
        {[0,1,2].map(i=>(
          <div key={i} style={{ width:10, height:10, borderRadius:"50%", background:"#e8a020", animation:`bounce 1s ${i*0.2}s infinite` }}/>
        ))}
      </div>
    ...
    <div style={{ padding:pad }}>
      <div style={{ display:"flex", gap:10, marginBottom:14, flexWrap:"wrap", alignItems:"center" }}>
        ...
        {!mob && <>
          ...
          <button onClick={()=>setShowModal("confirm")} style={{ background: pendingCount>0?"#e8a020":"#b0b0b0", color:"#fff", border:"none", borderRadius:9, padding:"11px 18px", fontFamily:"inherit", fontWeight:800, fontSize:14, cursor:"pointer", animation:pendingCount>0?"pulse 1.5s infinite":"none" }}>💾 บันทึก{pendingCount>0?` (${pendingCount})`:""}</button>
        </>}
      </div>
  ...
  const [logAction,      setLogAction]      = useState(null); // { type:"edit"|"delete", log }
  const [editLogData,    setEditLogData]    = useState(null); // { staff, date }
  ...
  const openLogEdit = (log) => {
    ...
    setLogAction({ type:"edit", log });
    setShowModal("logPin");
  };
  const openLogDelete = (log) => {
    setLogPinUnlocked(false); setLogPinInput(""); setLogPinError(false);
    setLogAction({ type:"delete", log });
    setShowModal("logPin");
  };
  const doDeleteLog = (id) => {
    sync(async () => {
      await remove(ref(db, `logs/${id}`));
      toast$("ลบประวัติสำเร็จ");
                ...
                <div key={log.id} style={{ background:"#fff", borderRadius:14, overflow:"hidden", boxShadow:"0 2px 8px rgba(0,0,0,.07)" }}>
                  <div style={{ background:"#1c1c1c", padding:"11px 16px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:6, flexShrink:0 }}>
                      ...
                      <button onClick={()=>openLogEdit(log)} style={{ background:"#2a2a2a", border:"none", borderRadius:7, width:30, height:30, cursor:"pointer", fontSize:13, display:"flex", alignItems:"center", justifyContent:"center" }} title="แก้ไข">✏️</button>
                      <button onClick={()=>openLogDelete(log)} style={{ background:"#3a1a1a", border:"none", borderRadius:7, width:30, height:30, cursor:"pointer", fontSize:13, display:"flex", alignItems:"center", justifyContent:"center" }} title="ลบ">🗑️</button>
                    </div>
                  </div>
  ...
  const SummaryPage = () => {
        ...
        <div style={{ background:"#fff", borderRadius:22, padding:"36px 28px", width:"100%", maxWidth:340, textAlign:"center", boxShadow:"0 8px 40px rgba(0,0,0,.12)" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:9, marginBottom:9 }}>
            {[1,2,3,4,5,6,7,8,9].map(n=>(
              <button key={n} onClick={()=>pressDigit(String(n))} style={{ padding:"16px 0", borderRadius:12, border:"1.5px solid #ebebeb", background:"#fafafa", fontFamily:"inherit", fontSize:20, fontWeight:700, cursor:"pointer", color:"#1c1c1c" }}>{n}</button>
            ))}
    ...
    return (
      <div style={{ padding:pad }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:mob?"repeat(2,1fr)":"repeat(4,1fr)", gap:11, marginBottom:22 }}>
          {[{l:"สินค้าทั้งหมด",v:items.length,i:"📦",c:"#1c1c1c"},{l:"น้อยมาก",v:lowItems.length,i:"🔴",c:"#c62828"},{l:"ปกติ",v:items.filter(i=>i.status==="normal").length,i:"🟡",c:"#e65100"},{l:"มีมาก",v:items.filter(i=>i.status==="high").length,i:"🟢",c:"#2e7d32"}].map(s=>(
            <div key={s.l} style={{ background:"#fff", borderRadius:13, padding:15, boxShadow:"0 2px 8px rgba(0,0,0,.07)" }}>
              <div style={{ fontSize:22 }}>{s.i}</div>
              <div style={{ fontSize:26, fontWeight:800, color:s.c, marginTop:3 }}>{s.v}</div>
              <div style={{ fontSize:12, color:"#888", marginTop:2 }}>{s.l}</div>
            </div>
          ))}
        </div>
        <div style={{ background:"#fff", borderRadius:13, overflow:"hidden", boxShadow:"0 2px 8px rgba(0,0,0,.07)", marginBottom:22 }}>
          <div style={{ background:"#1c1c1c", padding:"10px 16px" }}><span style={{ color:"#fff", fontWeight:700, fontSize:14 }}>รายการสต๊อคทั้งหมด</span></div>
          {sortedItems.map((item,idx)=>{
            const col=getCC(item.category),st=getSt(item.status),zc=getZC(item.zone);
            return (
              <div key={item.id} style={{ padding:"11px 16px", borderBottom:"1px solid #f0f0f0", display:"flex", alignItems:"center", gap:12, background:idx%2===0?"#fff":"#fafafa" }}>
                <span style={{ fontSize:11, color:"#ddd", width:18 }}>{idx+1}</span>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontWeight:700, fontSize:14 }}>{item.name}</div>
                  <div style={{ display:"flex", gap:5, marginTop:3, flexWrap:"wrap" }}>
                    <span style={{ background:col.bg, color:col.accent, borderRadius:5, padding:"1px 6px", fontSize:10, fontWeight:600 }}>{item.category}</span>
                    <span style={{ background:zc.bg, color:zc.accent, borderRadius:5, padding:"1px 6px", fontSize:10, fontWeight:600 }}>{zc.icon} {item.zone}</span>
                  </div>
                </div>
                <div style={{ textAlign:"right", flexShrink:0 }}>
                  <div style={{ fontWeight:800, fontSize:16 }}>{item.qty} <span style={{ fontSize:11, color:"#bbb", fontWeight:500 }}>{item.unit}</span></div>
                  <span style={{ background:st.bg, color:st.color, borderRadius:5, padding:"1px 7px", fontSize:11, fontWeight:700 }}>{st.icon} {st.label}</span>
                </div>
              </div>
            );
        ...
        {summaryLogs.length>0 && <div>
          <h3 style={{ fontWeight:700, fontSize:14, marginBottom:10 }}>📝 บันทึกวันที่ {summaryDate}</h3>
          {summaryLogs.map(log=>(
            <div key={log.id} style={{ background:"#fff", borderRadius:10, padding:"10px 15px", marginBottom:7, boxShadow:"0 1px 4px rgba(0,0,0,.06)" }}>
              <span style={{ fontWeight:700, color:"#e8a020" }}>👤 {log.staff}</span>
              <span style={{ color:"#aaa", fontSize:11, marginLeft:10 }}>🕐 {log.timestamp}</span>
              <div style={{ fontSize:13, color:"#555", marginTop:5 }}>{log.changes.filter(c=>c.delta!==0).map(c=>`${c.itemName} (${c.delta>0?"+":""}${c.delta})`).join(", ")||"ไม่มีการเปลี่ยนแปลง"}</div>
            </div>
          ))}
        </div>}
      </div>
  ...
  const SettingsPage = () => (
    <div style={{ padding:pad }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:mob?"1fr":"repeat(2,1fr)", gap:13, alignItems:"start" }}>
        <SettingsSection title="📍 โซนทำงาน" inputVal={newZoneInput} setInput={setNewZoneInput} onAdd={addZone} placeholder="ชื่อโซนใหม่..." chips={zones.map(z=>{const zc=getZC(z),inUse=items.some(i=>i.zone===z);return <Chip key={z} bg={zc.bg} accent={zc.accent} light={zc.light} onDelete={!inUse?()=>delZone(z):null}>{zc.icon} {z}{inUse&&<span style={{fontSize:9,color:"#bbb",marginLeft:3}}>ใช้งาน</span>}</Chip>;})} />
        <SettingsSection title="🗂️ ประเภทสินค้า" inputVal={newCatInput} setInput={setNewCatInput} onAdd={addCat} placeholder="ชื่อประเภทใหม่..." chips={categories.map(c=>{const col=getCC(c),inUse=items.some(i=>i.category===c);return <Chip key={c} bg={col.bg} accent={col.accent} light={col.light} onDelete={!inUse?()=>delCat(c):null}>{c}{inUse&&<span style={{fontSize:9,color:"#bbb",marginLeft:3}}>ใช้งาน</span>}</Chip>;})} />
        <SettingsSection title="📐 หน่วยนับ" inputVal={newUnitInput} setInput={setNewUnitInput} onAdd={addUnit} placeholder="หน่วยใหม่..." chips={units.map(u=>{const inUse=items.some(i=>i.unit===u);return <Chip key={u} bg="#f0f4ff" accent="#3949ab" light="#c5cae9" onDelete={!inUse?()=>delUnit(u):null}>{u}{inUse&&<span style={{fontSize:9,color:"#bbb",marginLeft:3}}>ใช้งาน</span>}</Chip>;})} />
        <div style={{ background:"#fff", borderRadius:13, padding:17, boxShadow:"0 2px 8px rgba(0,0,0,.07)" }}>
  ...
  return (
    <div style={{ fontFamily:"'Sarabun','Noto Sans Thai',sans-serif", background:"#f5f0eb", minHeight:"100vh", color:"#1a1a1a", display:"flex", flexDirection:"column" }}>
      ...
      {/* Syncing indicator */}
      {syncing && (
        <div style={{ position:"fixed", top:0, left:0, right:0, height:3, background:"linear-gradient(90deg,#e8a020,#f5c842,#e8a020)", backgroundSize:"200% 100%", animation:"shimmer 1s infinite", zIndex:10000 }}/>
      )}
        ...
        {!mob && (
          <aside style={{ width:196, background:"#fff", borderRight:"1px solid #ede8e2", flexShrink:0, display:"flex", flexDirection:"column", overflowY:"auto" }}>
            ...
            <div style={{ margin:"9px 11px 14px", padding:"8px 10px", background:"#e8f5e9", borderRadius:9, border:"1px solid #a5d6a7", display:"flex", alignItems:"center", gap:7 }}>
              ...
              <div>
                <div style={{ fontSize:10, fontWeight:700, color:"#2e7d32" }}>Firebase Connected</div>
                <div style={{ fontSize:9, color:"#81c784" }}>Real-time sync</div>
              </div>
            </div>
          </aside>
        )}
            ...
            {unchangedItems.length>0 && (
              <div style={{ marginBottom:16 }}>
                <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:7 }}>
                  ...
                  <span style={{ fontSize:11, color:"#bbb", fontWeight:500 }}>— ไม่มีการแก้ไข</span>
                </div>
        ...
        <Modal maxW={340}>
          <div style={{ textAlign:"center" }}>
            ...
            <div style={{ fontWeight:800, fontSize:17, marginBottom:4 }}>
              {logAction?.type==="delete" ? "ลบประวัติ" : "แก้ไขประวัติ"}
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:9, marginBottom:9 }}>
              {[1,2,3,4,5,6,7,8,9].map(n=>(
                <button key={n} onClick={()=>pressLogPin(String(n))} style={{ padding:"14px 0", borderRadius:12, border:"1.5px solid #ebebeb", background:"#fafafa", fontFamily:"inherit", fontSize:20, fontWeight:700, cursor:"pointer", color:"#1c1c1c" }}>{n}</button>
              ))}
      ...
      {/* ── Log Edit Modal (after PIN unlock) ── */}
      {showModal==="logPin" && logPinUnlocked && logAction?.type==="edit" && editLogData && (
        <Modal maxW={400}>
          ...
          <div style={{ background:"#f7f7f7", borderRadius:10, padding:"10px 13px", marginBottom:16 }}>
            <div style={{ fontSize:11, fontWeight:700, color:"#aaa", marginBottom:8, textTransform:"uppercase", letterSpacing:.5 }}>รายการที่เปลี่ยนแปลง (ไม่สามารถแก้ไขได้)</div>
