...
function Chip({bg,accent,light,children,onDelete}){
  return(
    <div style={{display:"flex",alignItems:"center",gap:5,background:bg,borderRadius:50,padding:"6px 10px 6px 13px",border:"1.5px solid "+light}}>
      ...
      {onDelete&&<button onClick={onDelete} style={{background:"rgba(0,0,0,.1)",border:"none",borderRadius:"50%",width:17,height:17,cursor:"pointer",color:"#c62828",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,flexShrink:0}}>×</button>}
    </div>
  );
}

function Modal({children,maxW=500,mob,onClose}){
  return(
    <div onClick={e=>{if(e.target===e.currentTarget)onClose();}} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",zIndex:1000,display:"flex",alignItems:mob?"flex-end":"center",justifyContent:"center",padding:mob?0:24}}>
...
function PinPad({pinInput,pinError,onPress,onBack,onCancel,title,subtitle,icon}){
  return(
    <div style={{textAlign:"center"}}>
      <div style={{fontSize:40,marginBottom:8}}>{icon}</div>
      <div style={{fontWeight:800,fontSize:17,marginBottom:4}}>{title}</div>
      <div style={{fontSize:13,color:"#aaa",marginBottom:22}}>{subtitle}</div>
      <div style={{display:"flex",justifyContent:"center",gap:10,marginBottom:22}}>
        {[0,1,2,3,4,5].map(i=><div key={i} style={{width:13,height:13,borderRadius:"50%",background:i<pinInput.length?(pinError?"#c62828":"#1c1c1c"):"#e0e0e0",transition:"background .15s"}}/>)}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:9,marginBottom:9}}>
        {[1,2,3,4,5,6,7,8,9].map(n=><button key={n} onClick={()=>onPress(String(n))} style={{padding:"14px 0",borderRadius:12,border:"1.5px solid #ebebeb",background:"#fafafa",fontFamily:"inherit",fontSize:20,fontWeight:700,cursor:"pointer",color:"#1c1c1c"}}>{n}</button>)}
        <div/><button onClick={()=>onPress("0")} style={{padding:"14px 0",borderRadius:12,border:"1.5px solid #ebebeb",background:"#fafafa",fontFamily:"inherit",fontSize:20,fontWeight:700,cursor:"pointer",color:"#1c1c1c"}}>0</button>
        <button onClick={onBack} style={{padding:"14px 0",borderRadius:12,border:"1.5px solid #ebebeb",background:"#fafafa",fontFamily:"inherit",fontSize:20,cursor:"pointer",color:"#888"}}>⌫</button>
      </div>
      {pinError&&<div style={{color:"#c62828",fontWeight:700,fontSize:13,marginBottom:8}}>PIN ไม่ถูกต้อง</div>}
      {onCancel&&<button onClick={onCancel} style={{width:"100%",padding:12,borderRadius:11,border:"1.5px solid #ddd",background:"#fff",fontFamily:"inherit",fontWeight:700,cursor:"pointer",fontSize:14,marginTop:4}}>ยกเลิก</button>}
    </div>
  );
}

// ─── ItemCard (stable, outside App) ─────────────────────────────
function ItemCard({item,delta,mob,onMinus,onPlus,onDeltaChange,onEdit,onDelete,onStatus}){
    ...
    <div style={{background:"#fff",borderRadius:14,overflow:"hidden",boxShadow:changed?"0 0 0 2px "+zc.accent+",0 4px 16px rgba(0,0,0,.1)":"0 1px 8px rgba(0,0,0,.07)",borderLeft:"4px solid "+st.border}}>
      <div style={{padding:"12px 13px 9px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
          ...
          <div style={{display:"flex",gap:5,marginLeft:8,flexShrink:0}}>
            <button onClick={()=>onEdit(item)} style={{background:"#f5f5f5",border:"none",borderRadius:7,width:34,height:34,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>✏️</button>
            <button onClick={()=>onDelete(item.id)} style={{background:"#fff0f0",border:"none",borderRadius:7,width:34,height:34,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>🗑️</button>
          </div>
        </div>
        <div style={{display:"flex",gap:5}}>
          {Object.entries(STOCK_STATUS).map(([k,v])=>(
            <button key={k} onClick={()=>onStatus(item.id,k)} style={{flex:1,padding:"6px 3px",borderRadius:8,border:item.status===k?"2px solid "+v.color:"1.5px solid #e8e8e8",background:item.status===k?v.bg:"#fafafa",cursor:"pointer",fontFamily:"inherit",fontSize:11,fontWeight:item.status===k?800:400,color:item.status===k?v.color:"#bbb"}}>
              {v.icon} {v.label}
            </button>
          ))}
        </div>
      ...
      <div style={{background:zc.bg,padding:"10px 13px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <button onClick={()=>onMinus(item.id)} style={{width:mob?42:36,height:mob?42:36,borderRadius:9,background:"#fff",border:"2px solid "+zc.light,cursor:"pointer",fontSize:18,fontWeight:800,color:zc.accent,display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
          <input
            type="number"
            value={delta===0?"":delta}
...
export default function App(){
        ...
        return(
          <div key={zone} style={{marginBottom:26}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"repeat(auto-fill,minmax(300px,1fr))",gap:11}}>
              {zoneItems.map(item=>(
                <ItemCard key={item.id} item={item} delta={pendingChanges[item.id]??0} mob={mob}
              ...
              <div key={log.id} style={{background:"#fff",borderRadius:14,overflow:"hidden",boxShadow:"0 2px 8px rgba(0,0,0,.07)"}}>
                ...
                <div style={{padding:"12px 16px"}}>
                  {changed.length>0&&<div style={{marginBottom:10}}>
                    ...
                    {changed.map((c,i)=>{const col=getCC(c.category);return(
  ...
  const SortBtn=({col,label})=>{
    ...
    return<button onClick={()=>handleSort(col)} style={{background:"none",border:"none",cursor:"pointer",fontFamily:"inherit",fontWeight:active?800:600,fontSize:12,color:active?"#e8a020":"#aaa",display:"inline-flex",alignItems:"center",gap:3,padding:0,whiteSpace:"nowrap"}}>
    ...
    if(!pinUnlocked)return(
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"70vh",padding:16}}>
        <div style={{background:"#fff",borderRadius:22,padding:"36px 28px",width:"100%",maxWidth:340,textAlign:"center",boxShadow:"0 8px 40px rgba(0,0,0,.12)"}}>
          ...
          <PinPad pinInput={pinInput} pinError={pinError}
      ...
      <div style={{padding:pad}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:10}}>
          <div>
            ...
            <div style={{fontSize:12,color:"#aaa",marginTop:3}}>อัปเดต: {new Date().toLocaleString("th-TH")}</div>
          </div>
        ...
        <div style={{marginBottom:8}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
            ...
            <span style={{fontWeight:800,fontSize:15}}>ภาพรวมสต๊อค</span>
          </div>
          <div style={{display:"grid",gridTemplateColumns:mob?"repeat(2,1fr)":"repeat(4,1fr)",gap:11,marginBottom:24}}>
            {[{l:"สินค้าทั้งหมด",v:items.length,c:"#1c1c1c",bg:"#f5f5f5",border:"#e0e0e0"},{l:"น้อยมาก",v:lowItems.length,c:"#c62828",bg:"#ffebee",border:"#ef9a9a"},{l:"ปกติ",v:items.filter(i=>i.status==="normal").length,c:"#e65100",bg:"#fff8f0",border:"#ffcc80"},{l:"มีมาก",v:items.filter(i=>i.status==="high").length,c:"#2e7d32",bg:"#e8f5e9",border:"#a5d6a7"}].map(s=>(
              <div key={s.l} style={{background:s.bg,borderRadius:13,padding:"14px 16px",boxShadow:"0 2px 8px rgba(0,0,0,.06)",border:"1.5px solid "+s.border}}>
        ...
        <div style={{marginBottom:24}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
            ...
            <span style={{fontWeight:800,fontSize:15}}>รายการสต๊อคทั้งหมด</span>
            <span style={{fontSize:12,color:"#aaa"}}>({items.length} รายการ)</span>
          </div>
          <div style={{background:"#fff",borderRadius:13,overflow:"hidden",boxShadow:"0 2px 8px rgba(0,0,0,.07)"}}>
            ...
            {sortedSummaryItems.map((item,idx)=>{
              ...
              return(
                <div key={item.id} style={{padding:"11px 16px",borderBottom:"1px solid #f5f5f5",display:"grid",gridTemplateColumns:mob?"2fr 1fr 1fr":"3fr 1fr 1fr 1fr 1fr",gap:8,alignItems:"center",background:isLow?"#fff8f8":idx%2===0?"#fff":"#fafafa"}}>
                  ...
                  <div style={{fontWeight:800,fontSize:15}}>{item.qty} <span style={{fontSize:11,color:"#bbb",fontWeight:500}}>{item.unit}</span></div>
        ...
        <div>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
            ...
            <span style={{fontWeight:800,fontSize:15}}>บันทึกวันที่ {summaryDate}</span>
            <span style={{fontSize:12,color:"#aaa"}}>({summaryLogs.length} รายการ)</span>
          </div>
          {summaryLogs.length===0
            ?<div style={{background:"#fff",borderRadius:13,padding:"28px 16px",textAlign:"center",color:"#ccc",boxShadow:"0 2px 8px rgba(0,0,0,.07)"}}><div style={{fontSize:36,marginBottom:8}}>📭</div><div style={{fontSize:14}}>ไม่มีบันทึกในวันนี้</div></div>
            :summaryLogs.map(log=>{
              ...
              return(
                <div key={log.id} style={{background:"#fff",borderRadius:13,overflow:"hidden",marginBottom:10,boxShadow:"0 2px 8px rgba(0,0,0,.07)"}}>
                  <div style={{background:"#1c1c1c",padding:"10px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div><span style={{color:"#e8a020",fontWeight:800,fontSize:14}}>👤 {log.staff}</span><span style={{color:"#666",fontSize:11,marginLeft:10}}>🕐 {log.timestamp}</span></div>
                    <span style={{background:"#2a2a2a",color:"#888",fontSize:11,borderRadius:7,padding:"3px 9px"}}>{logChanged.length} รายการ</span>
                  </div>
                  <div style={{padding:"12px 16px"}}>
                    {logChanged.length===0?<div style={{fontSize:13,color:"#ccc"}}>ไม่มีการเปลี่ยนแปลง</div>
                      :logChanged.map((c,i)=>{const cc=getCC(c.category);return(
  ...
  const renderSettings=()=>(
    <div style={{padding:pad}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"repeat(2,1fr)",gap:13,alignItems:"start"}}>
        {[
          {title:"📍 โซนทำงาน",val:newZoneInput,setVal:setNewZoneInput,onAdd:addZone,ph:"ชื่อโซนใหม่...",chips:zones.map(z=>{const zc=getZC(z),inUse=items.some(i=>i.zone===z);return<Chip key={z} bg={zc.bg} accent={zc.accent} light={zc.light} onDelete={!inUse?()=>delZone(z):null}>{zc.icon} {z}</Chip>;})},
          {title:"🗂️ ประเภทสินค้า",val:newCatInput,setVal:setNewCatInput,onAdd:addCat,ph:"ชื่อประเภทใหม่...",chips:categories.map(c=>{const col=getCC(c),inUse=items.some(i=>i.category===c);return<Chip key={c} bg={col.bg} accent={col.accent} light={col.light} onDelete={!inUse?()=>delCat(c):null}>{c}</Chip>;})},
          {title:"📐 หน่วยนับ",val:newUnitInput,setVal:setNewUnitInput,onAdd:addUnit,ph:"หน่วยใหม่...",chips:units.map(u=>{const inUse=items.some(i=>i.unit===u);return<Chip key={u} bg="#f0f4ff" accent="#3949ab" light="#c5cae9" onDelete={!inUse?()=>delUnit(u):null}>{u}</Chip>;})},
        ].map(sec=>(
          <div key={sec.title} style={{background:"#fff",borderRadius:13,padding:17,boxShadow:"0 2px 8px rgba(0,0,0,.07)"}}>
            <div style={{fontWeight:700,fontSize:14,marginBottom:11}}>{sec.title}</div>
            <div style={{display:"flex",gap:8,marginBottom:11}}>
              ...
              <button onClick={sec.onAdd} style={{background:"#1c1c1c",color:"#fff",border:"none",borderRadius:9,padding:"0 15px",fontFamily:"inherit",fontWeight:700,cursor:"pointer",fontSize:14,flexShrink:0}}>+ เพิ่ม</button>
            </div>
        ...
        <div style={{background:"#fff",borderRadius:13,padding:17,boxShadow:"0 2px 8px rgba(0,0,0,.07)"}}>
          ...
          {changePinStep==="new"&&<div>
            ...
            <div style={{display:"flex",gap:7}}>
              <input type="password" maxLength={6} value={changePinVal} onChange={e=>setChangePinVal(e.target.value.replace(/\D/g,""))} placeholder="• • • • • •" style={{...inputSt,flex:1,fontSize:18,letterSpacing:8}}/>
              <button onClick={()=>{if(changePinVal.length===6){saveSettings({ownerPin:changePinVal});setChangePinStep(null);setChangePinVal("");toast;}}} style={{background:"#2e7d32",color:"#fff",border:"none",borderRadius:9,padding:"0 14px",fontFamily:"inherit",fontWeight:700,cursor:"pointer",fontSize:14}}>บันทึก</button>
  ...
  return(
    <div style={{fontFamily:"'Sarabun','Noto Sans Thai',sans-serif",background:"#f5f0eb",minHeight:"100vh",color:"#1a1a1a",display:"flex",flexDirection:"column"}}>
      ...
      {toast&&<div style={{position:"fixed",top:16,left:"50%",transform:"translateX(-50%)",zIndex:9999,background:toast.type==="error"?"#c62828":"#1c1c1c",color:"#fff",padding:"11px 22px",borderRadius:50,fontWeight:700,fontSize:14,boxShadow:"0 4px 24px rgba(0,0,0,.25)",animation:"slideDown .25s ease",whiteSpace:"nowrap"}}>
      ...
      <header style={{background:"#1c1c1c",height:mob?52:58,display:"flex",alignItems:"center",justifyContent:"space-between",padding:mob?"0 16px":"0 28px",flexShrink:0,position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 12px rgba(0,0,0,.3)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            <div style={{color:"#fff",fontWeight:800,fontSize:mob?14:16,lineHeight:1.1}}>จัดการสต๊อคร้าน YuGrill</div>
        ...
        {!mob&&(
          <nav style={{display:"flex",alignItems:"center",gap:3}}>
            ...
            {page==="stock"&&<button onClick={()=>setShowModal("confirm")} style={{background:pendingCount>0?"#e8a020":"#b0b0b0",color:"#fff",border:"none",borderRadius:8,padding:"7px 15px",fontFamily:"inherit",fontWeight:800,fontSize:14,cursor:"pointer",marginLeft:6,animation:pendingCount>0?"pulse 1.5s infinite":"none"}}>
              💾 บันทึก{pendingCount>0?" ("+pendingCount+")":""}
            </button>}
          </nav>
        )}
        {mob&&page==="stock"&&<button onClick={()=>setShowModal("confirm")} style={{background:pendingCount>0?"#e8a020":"#b0b0b0",border:"none",borderRadius:9,padding:"7px 12px",cursor:"pointer",fontFamily:"inherit",fontWeight:800,fontSize:13,color:"#fff",display:"flex",alignItems:"center",gap:5,animation:pendingCount>0?"pulse 1.5s infinite":"none"}}>
      ...
      {lowItems.length>0&&page==="stock"&&<div style={{background:"#ffebee",borderBottom:"2px solid #ef9a9a",padding:"8px 16px",display:"flex",alignItems:"center",gap:8}}>
        <span>🔴</span><span style={{fontWeight:700,color:"#c62828",fontSize:13}}>น้อยมาก: </span>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        {!mob&&<aside style={{width:196,background:"#fff",borderRight:"1px solid #ede8e2",flexShrink:0,display:"flex",flexDirection:"column",overflowY:"auto"}}>
          ...
          {lowItems.length>0&&<div style={{margin:"9px 11px 0",padding:"9px",background:"#ffebee",borderRadius:9,border:"1px solid #ef9a9a"}}>
      ...
      {mob&&<nav style={{position:"fixed",bottom:0,left:0,right:0,background:"#fff",borderTop:"1px solid #ebebeb",display:"grid",gridTemplateColumns:"repeat(4,1fr)",zIndex:200,boxShadow:"0 -2px 14px rgba(0,0,0,.07)"}}>
        {NAV.map(n=>(
      ...
      {/* ── Item Modal ── */}
      {(showModal==="item")&&<Modal mob={mob} onClose={closeModal} maxW={500}>
        <h3 style={{fontWeight:800,fontSize:18,marginBottom:16}}>{editItem?"✏️ แก้ไขสินค้า":"➕ เพิ่มสินค้าใหม่"}</h3>
        <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr",gap:12}}>
          <div><label style={{fontSize:12,color:"#555",fontWeight:600,display:"block",marginBottom:5}}>ชื่อสินค้า *</label>
            <input value={formData.name} onChange={e=>setFormData(p=>({...p,name:e.target.value}))} {...inputAttr} style={inputSt}/></div>
          <div><label style={{fontSize:12,color:"#555",fontWeight:600,display:"block",marginBottom:5}}>จำนวน *</label>
            <input type="number" value={formData.qty} onChange={e=>setFormData(p=>({...p,qty:e.target.value}))} style={inputSt}/></div>
          <div><label style={{fontSize:12,color:"#555",fontWeight:600,display:"block",marginBottom:5}}>โซนทำงาน *</label>
            <select value={formData.zone} onChange={e=>setFormData(p=>({...p,zone:e.target.value}))} style={inputSt}>{zones.map(z=><option key={z} value={z}>{getZC(z).icon} {z}</option>)}</select></div>
          <div><label style={{fontSize:12,color:"#555",fontWeight:600,display:"block",marginBottom:5}}>ประเภท *</label>
          ...
          <div><label style={{fontSize:12,color:"#555",fontWeight:600,display:"block",marginBottom:5}}>หน่วย *</label>
          ...
          <div><label style={{fontSize:12,color:"#555",fontWeight:600,display:"block",marginBottom:7}}>สถานะ *</label>
            <div style={{display:"flex",gap:6}}>
              {Object.entries(STOCK_STATUS).map(([k,v])=>(
                <button key={k} type="button" onClick={()=>setFormData(p=>({...p,status:k}))} style={{flex:1,padding:"10px 3px",borderRadius:9,border:formData.status===k?"2px solid "+v.color:"1.5px solid #e8e8e8",background:formData.status===k?v.bg:"#fafafa",cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:formData.status===k?800:400,color:formData.status===k?v.color:"#bbb"}}>
                  {v.icon}<br/>{v.label}
                </button>
              ))}
            </div>
        ...
        <div style={{display:"flex",gap:10,marginTop:20}}>
          <button onClick={closeModal} style={{flex:1,padding:13,borderRadius:11,border:"1.5px solid #ddd",background:"#fff",fontFamily:"inherit",fontWeight:700,cursor:"pointer",fontSize:15}}>ยกเลิก</button>
      ...
      {showModal==="confirm"&&(()=>{
        ...
        return<Modal mob={mob} onClose={closeModal} maxW={500}>
          ...
          <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr",gap:10,marginBottom:16}}>
            <div><label style={{fontSize:12,fontWeight:600,color:"#555",display:"block",marginBottom:5}}>ชื่อพนักงาน *</label>
              <input placeholder="กรอกชื่อ..." value={staffName} onChange={e=>setStaffName(e.target.value)} {...inputAttr} style={inputSt}/></div>
            <div><label style={{fontSize:12,fontWeight:600,color:"#555",display:"block",marginBottom:5}}>วันที่บันทึก</label>
          ...
          {changedItems.length>0&&<div style={{marginBottom:12}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:7}}>
              <div style={{width:3,height:14,background:"#2e7d32",borderRadius:2}}/><span style={{fontSize:12,fontWeight:800,color:"#2e7d32",textTransform:"uppercase",letterSpacing:.5}}>เปลี่ยนแปลง</span>
            </div>
            <div style={{background:"#f1faf3",border:"1.5px solid #c8e6c9",borderRadius:11,overflow:"hidden"}}>
              {changedItems.map((it,i)=>{const d=pendingChanges[it.id],isAdd=d>0;return(
                <div key={it.id} style={{display:"flex",alignItems:"center",padding:"10px 13px",borderBottom:i<changedItems.length-1?"1px solid #c8e6c9":"none",gap:10}}>
                  ...
                  <div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,fontSize:14}}>{it.name}</div><div style={{fontSize:11,color:"#888"}}>{it.qty} → {it.qty+d} {it.unit}</div></div>
          ...
          {unchangedItems.length>0&&<div style={{marginBottom:16}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:7}}>
              <div style={{width:3,height:14,background:"#9e9e9e",borderRadius:2}}/><span style={{fontSize:12,fontWeight:800,color:"#9e9e9e",textTransform:"uppercase",letterSpacing:.5}}>คงที่</span>
            </div>
            <div style={{background:"#f0f0f0",border:"1.5px solid #d8d8d8",borderRadius:11,overflow:"hidden"}}>
              {unchangedItems.map((it,i)=>(
                <div key={it.id} style={{display:"flex",alignItems:"center",padding:"9px 13px",borderBottom:i<unchangedItems.length-1?"1px solid #e0e0e0":"none",gap:10,background:i%2===0?"#f0f0f0":"#ebebeb"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}><div style={{fontWeight:600,fontSize:13,color:"#777"}}>{it.name}</div><div style={{fontSize:11,color:"#aaa"}}>{it.qty} {it.unit}</div></div>
      ...
      {/* ── Log PIN Modal ── */}
      {showModal==="logPin"&&!logPinUnlocked&&<Modal mob={mob} onClose={()=>{setShowModal(null);setLogAction(null);setLogPinInput("");}} maxW={340}>
        <PinPad pinInput={logPinInput} pinError={logPinError}
      ...
      {/* ── Log Edit Modal ── */}
      {showModal==="logPin"&&logPinUnlocked&&logAction?.type==="edit"&&editLogData&&<Modal mob={mob} onClose={()=>{setShowModal(null);setLogAction(null);setLogPinUnlocked(false);setEditLogData(null);}} maxW={400}>
      ...
      {/* ── Log Delete Modal ── */}
      {showModal==="logPin"&&logPinUnlocked&&logAction?.type==="delete"&&<Modal mob={mob} onClose={()=>{setShowModal(null);setLogAction(null);setLogPinUnlocked(false);}} maxW={360}>
